...
public class AffineTransform {
    ...
    public AffineTransform createInverse() throws RuntimeException {
        double det = getDeterminant();
        if (Math.abs(det) < ZERO) {
            // awt.204=Determinant is zero
            throw new RuntimeException("not invertable"); //$NON-NLS-1$
        }
        return new AffineTransform(
                 m11 / det, // m00
                -m10 / det, // m10
    ...
    public Point2D inverseTransform(Point2D src, Point2D dst) throws RuntimeException {
        double det = getDeterminant();
        if (Math.abs(det) < ZERO) {
            // awt.204=Determinant is zero
            throw new RuntimeException("not invertible"); //$NON-NLS-1$
        }
